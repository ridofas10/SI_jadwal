...
$query = "
    SELECT 
        ...
        jb.nama_mahasiswa,
        jb.hari,
        DATE_FORMAT(jb.tanggal, '%Y-%m-%d') AS tanggal,
        jb.jam,
        jb.ruangan,
        jb.materi
...
// Logika untuk membatasi pencetakan setelah tanggal bimbingan
$current_date = date('Y-m-d'); // Tanggal hari ini
$bimbingan_date = $row['tanggal']; // Tanggal bimbingan dari database

if ($current_date > $bimbingan_date) {
    echo "<script>alert('Pencetakan tidak diizinkan karena tanggal bimbingan telah lewat.');</script>";
